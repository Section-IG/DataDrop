...
module.exports = {
	...
	async execute(client, log, message, args) {
		...
		await message.reply(
			"Ceci est une preview de l'annonce. Tapez 'yes' pour l'envoyer ou tout autre chose pour annuler.",
			embed
		);
		...
		if (confirmation && confirmation.content.toLowerCase() === 'yes') {
			const annoncesChannel = message.guild.channels.cache.get(
				announce.channelid
			);
			const annoncesRole = message.guild.roles.cache.get(announce.roleid);
			const announceMessage = await annoncesChannel.send(
				annoncesRole.toString(),
				embed
			);
