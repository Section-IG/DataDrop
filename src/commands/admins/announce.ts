    ...
    async execute(
        ...
        try {
            ...
            if (confirmation.customId.startsWith("ac-confirm")) {
                const annoncesChannel = await interaction.guild?.channels.fetch(
                    announce.channelid,
                );
                if (!annoncesChannel) {
                    await confirmation.update({
                        content:
                            "❌ **Oups!** - Le canal des annonces est introuvable.",
                        components: [],
                    });
                    return;
                }

                const annoncesRole = await interaction.guild?.roles.fetch(
                    announce.roleid,
                );
                const announceMessage = await (
                    annoncesChannel as TextChannel
                ).send({
