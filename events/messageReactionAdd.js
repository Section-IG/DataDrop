...
module.exports = async (client, log, messageReaction, user) => {
    ...
    if (emotes.includes(messageReaction.emoji.name) && message.channel.id === rolesChannel.id) {
        switch (messageReaction.emoji.name) {
            case au1Emote:
                member.roles.add(au1Role).catch(log.error);
                log.info(`Le rôle <${au1Role.name}> a été donné à <${member.user.tag}>`);
                break;
            case au2Emote:
                member.roles.add(au2Role).catch(log.error);
                log.info(`Le rôle <${au2Role.name}> a été donné à <${member.user.tag}>`);
                break;
            case au3Emote:
